document.addEventListener('DOMContentLoaded', async () => {
  ...
  setTimeout(() => {
    ...
    bookmarks.forEach(word => {
      ...
      card.className = 'bg-white rounded-2xl shadow-lg p-4 mb-4 cursor-pointer transition hover:shadow-xl relative';
      card.onclick = () => {
      ...
      card.innerHTML = `
        <div class="flex justify-between items-start">
          <div>
            <h2 class="text-xl font-bold text-blue-600">${word.en}</h2>
            <p class="text-lg text-gray-700">${word.ru}</p>
            <p class="text-sm text-gray-400">${word.pron || ''}</p>
          </div>
          <button onclick="event.stopPropagation(); removeBookmark(${word.id})"
            class="text-red-500 hover:text-red-600 absolute right-3 top-3">
